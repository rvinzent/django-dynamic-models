...
from dynamic_models.exceptions import ModelDoesNotExistError


def test_get_registered_model(model_schema):
    """Should find a model schema's model in the app registry."""
    assert utils.get_registered_model(model_schema) == model_schema.as_model()
    model_schema.factory.destroy()
    with pytest.raises(ModelDoesNotExistError):
        utils.get_registered_model(model_schema)
